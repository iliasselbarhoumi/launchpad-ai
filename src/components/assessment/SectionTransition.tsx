...
const SectionTransition: React.FC<SectionTransitionProps> = ({ completedSection, nextSection, onContinue }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-slate-600 text-lg mb-8 max-w-lg">
        {isFirstSection ? (
          <>
            First up is <strong>{nextSectionDetails.title}</strong>: <em>{nextSectionDetails.description}</em>.
          </>
        ) : (
          <>
            You've finished the <strong>{completedSection}</strong> section. Next up:{' '}
            <strong>{nextSectionDetails.title}: {nextSectionDetails.description}</strong>.
          </>
        )}
      </p>
      <Button onClick={onContinue} size="lg" className="bg-launchpad-purple hover:bg-launchpad-purple/90">
